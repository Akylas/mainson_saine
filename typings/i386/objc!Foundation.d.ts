...
declare class NSArray<ObjectType> extends NSObject implements CKRecordValue, NSCopying, NSFastEnumeration, NSMutableCopying, NSSecureCoding {
	...
	static arrayWithContentsOfDataError(data: NSData): NSArray<any>;

	static arrayWithContentsOfFile<ObjectType>(path: string): NSArray<ObjectType>;

	static arrayWithContentsOfFileError(path: string): NSArray<any>;
...
declare class NSAttributedString extends NSObject implements NSCopying, NSItemProviderReading, NSItemProviderWriting, NSMutableCopying, NSSecureCoding {
	...
	static attributedStringWithData(data: NSData): NSAttributedString;
	...
	static prefixForListItemWithCounterListStyleListIndentAttributes(listCounter: number, listStyle: DTCSSListStyle, listIndent: number, attributes: NSDictionary<any, any>): NSAttributedString;

	static synthesizedSmallCapsAttributedStringWithTextAttributes(text: string, attributes: NSDictionary<any, any>): NSAttributedString;
	...
	constructor(o: { HTMLData: NSData; baseURL: NSURL; documentAttributes: interop.Pointer | interop.Reference<NSDictionary<any, any>>; });

	constructor(o: { HTMLData: NSData; documentAttributes: interop.Pointer | interop.Reference<NSDictionary<any, any>>; });

	constructor(o: { HTMLData: NSData; options: NSDictionary<any, any>; documentAttributes: interop.Pointer | interop.Reference<NSDictionary<any, any>>; });
	...
	HTMLAttributesAtIndex(index: number): NSDictionary<any, any>;
	...
	dumpRangesOfAttribute(attribute: any): void;
	...
	initWithHTMLDataBaseURLDocumentAttributes(data: NSData, baseURL: NSURL, docAttributes: interop.Pointer | interop.Reference<NSDictionary<any, any>>): this;

	initWithHTMLDataDocumentAttributes(data: NSData, docAttributes: interop.Pointer | interop.Reference<NSDictionary<any, any>>): this;

	initWithHTMLDataOptionsDocumentAttributes(data: NSData, options: NSDictionary<any, any>, docAttributes: interop.Pointer | interop.Reference<NSDictionary<any, any>>): this;
	...
	itemNumberInTextListAtIndex(list: DTCSSListStyle, location: number): number;
	...
	rangeOfAnchorNamed(anchorName: string): NSRange;

	rangeOfFieldAtIndex(location: number): NSRange;

	rangeOfHTMLAttributeAtIndex(name: string, index: number): NSRange;

	rangeOfLinkAtIndexURL(location: number, URL: interop.Pointer | interop.Reference<NSURL>): NSRange;

	rangeOfTextBlockAtIndex(textBlock: DTTextBlock, location: number): NSRange;

	rangeOfTextListAtIndex(list: DTCSSListStyle, location: number): NSRange;
	...
	textAttachmentsWithPredicateClass(predicate: NSPredicate, theClass: typeof NSObject): NSArray<any>;
}
...
declare class NSBundle extends NSObject {
	...
	static nyt_photoViewerResourceBundle(): NSBundle;
...
declare class NSCharacterSet extends NSObject implements NSCopying, NSMutableCopying, NSSecureCoding {
	...
	static cssLengthUnitCharacterSet(): NSCharacterSet;

	static cssLengthValueCharacterSet(): NSCharacterSet;

	static cssStyleAttributeNameCharacterSet(): NSCharacterSet;

	static ignorableWhitespaceCharacterSet(): NSCharacterSet;

	static new(): NSCharacterSet; // inherited from NSObject

	static nonQuotedAttributeEndCharacterSet(): NSCharacterSet;

	static quoteCharacterSet(): NSCharacterSet;

	static tagAttributeNameCharacterSet(): NSCharacterSet;

	static tagNameCharacterSet(): NSCharacterSet;
...
declare class NSCoder extends NSObject {
	...
	decodeDTEdgeInsetsForKey(key: string): UIEdgeInsets;
	...
	encodeDTEdgeInsetsForKey(insets: UIEdgeInsets, key: string): void;
...
declare class NSData extends NSObject implements CKRecordValue, NSCopying, NSMutableCopying, NSSecureCoding {
	...
	static sd_UTTypeFromImageFormat(format: number): string;

	static sd_imageFormatForImageData(data: NSData): number;

	static sd_imageFormatFromUTType(uttype: string): number;
	...
	dataWithMD5Hash(): NSData;

	dataWithSHA1Hash(): NSData;

	decompressedDataUsingAlgorithmError(algorithm: NSDataCompressionAlgorithm): this;

	encodeWithCoder(coder: NSCoder): void;

	encryptedDataUsingSHA1WithKey(key: NSData): NSData;
...
declare class NSDictionary<KeyType, ObjectType> extends NSObject implements NSCopying, NSFastEnumeration, NSFastEnumeration, NSFetchRequestResult, NSMutableCopying, NSSecureCoding {
	...
	static dictionaryWithContentsOfDataError(data: NSData): NSDictionary<any, any>;

	static dictionaryWithContentsOfFile<KeyType, ObjectType>(path: string): NSDictionary<KeyType, ObjectType>;

	static dictionaryWithContentsOfFileError(path: string): NSDictionary<any, any>;
...
declare class NSFileWrapper extends NSObject implements NSSecureCoding {
	...
	fileWrapperByDeepCopying(): NSFileWrapper;
...
declare class NSMutableAttributedString extends NSAttributedString {
	...
	addHTMLAttributeValueRangeReplaceExisting(name: string, value: any, range: NSRange, replaceExisting: boolean): void;

	appendAttributedString(attrString: NSAttributedString): void;

	appendEndOfParagraph(): void;

	appendString(string: string): void;

	appendStringWithParagraphStyleFontDescriptor(string: string, paragraphStyle: DTCoreTextParagraphStyle, fontDescriptor: DTCoreTextFontDescriptor): void;
	...
	removeHTMLAttributeRange(name: string, range: NSRange): void;
...
declare class NSOperation extends NSObject implements SDWebImageOperation {
...
declare class NSScanner extends NSObject implements NSCopying {
	...
	scanCSSAttributeValue(name: interop.Pointer | interop.Reference<string>, value: interop.Pointer | interop.Reference<any>): boolean;

	scanCSSURL(urlString: interop.Pointer | interop.Reference<string>): boolean;
	...
	scanHTMLColor(color: interop.Pointer | interop.Reference<UIColor>): boolean;

	scanHTMLColorHTMLName(color: interop.Pointer | interop.Reference<UIColor>, name: interop.Pointer | interop.Reference<string>): boolean;
...
declare class NSString extends NSObject implements CKRecordValue, CNKeyDescriptor, NSCopying, NSItemProviderReading, NSItemProviderWriting, NSMutableCopying, NSSecureCoding {
	...
	static cachesPath(): string;

	static documentsPath(): string;
	...
	static stringWithUUID(): string;

	static temporaryPath(): string;
	...
	DTEdgeInsetsRelativeToCurrentTextSizeTextScale(textSize: number, textScale: number): UIEdgeInsets;

	arrayOfCSSShadowsWithCurrentTextSizeCurrentColor(textSize: number, color: UIColor): NSArray<any>;
	...
	dictionaryOfCSSStyles(): NSDictionary<any, any>;
	...
	hasPrefixCharacterFromSet(characterSet: NSCharacterSet): boolean;

	hasSuffix(str: string): boolean;

	hasSuffixCharacterFromSet(characterSet: NSCharacterSet): boolean;

	indexIsAtBeginningOfParagraph(index: number): boolean;
	...
	integerValueFromHex(): number;

	isCSSLengthValue(): boolean;
	...
	mdf_stringWithBidiEmbedding(): string;
	...
	pathByDeletingSequenceNumber(): string;

	pathByIncrementingSequenceNumber(): string;
	...
	pixelSizeOfCSSMeasureRelativeToCurrentTextSizeTextScale(textSize: number, textScale: number): number;
	...
	rangeOfParagraphAtIndex(index: number): NSRange;

	rangeOfParagraphsContainingRangeParBegIndexParEndIndex(range: NSRange, parBegIndex: interop.Pointer | interop.Reference<number>, parEndIndex: interop.Pointer | interop.Reference<number>): NSRange;
	...
	stringByAddingAppleConvertedSpace(): string;

	stringByAddingHTMLEntities(): string;
	...
	stringByReplacingHTMLEntities(): string;
...
declare class NSURL extends NSObject implements NSCopying, NSItemProviderReading, NSItemProviderWriting, NSSecureCoding, QLPreviewItem {
	...
	static appStoreReviewURLForApplicationIdentifier(identifier: string): NSURL;

	static appStoreURLforApplicationIdentifier(identifier: string): NSURL;
	...
	isEqualToURL(URL: NSURL): boolean;
	...
	unshortenWithCompletion(completion: (p1: NSURL) => void): void;
}
